...
Same UI as the test (menu_ui.py); run via LocalServer.vbs or: python menu_app.py
"""
...
class LocalServerMenu(ctk.CTk):
    def __init__(self):
        ...
        self.upload_progress_path = self.server_dir / "data/upload_progress.json"
        self._upload_progress_line = None
        self._upload_progress_last_state = None
        self._just_finished_upload = False
    ...
    def _build_logs_toggle(self):
        ...
        self.upload_progress_label.grid(row=2, column=0, sticky="ew", pady=(0, 4))
        self.logs_container.grid_columnconfigure(0, weight=1)
    ...
    def _update_upload_progress_ui(self):
        if hasattr(self, "upload_progress_label"):
            if self._upload_progress_line:
                self.upload_progress_label.configure(text=self._upload_progress_line)
                self.upload_progress_label.grid(row=2, column=0, sticky="ew", pady=(0, 4))
            else:
    ...
    def start_activity_monitor(self):
        def monitor():
            while True:
                try:
                    # Upload progress (real-time state for "Share to Server")
                    if self.upload_progress_path.exists():
                        try:
                            with open(self.upload_progress_path, "r", encoding="utf-8") as f:
                                prog = json.load(f)
                            state = prog.get("state", "")
                            filename = prog.get("filename", "?")
                            bytes_sent = prog.get("bytes_sent", 0)
                            bytes_total = prog.get("bytes_total", 1) or 1
                            if state == "uploading":
                                pct = min(100, int(100 * bytes_sent / bytes_total))
                                sent_h = self.human_size(bytes_sent)
                                total_h = self.human_size(bytes_total)
                                self._upload_progress_line = f"⬆ Uploading {filename}: {pct}% ({sent_h} / {total_h})"
                                self._upload_progress_last_state = "uploading"
                                self.after(0, self._update_upload_progress_ui)
                            elif state == "done":
                                if self._upload_progress_last_state == "uploading":
                                    self.add_activity_log(f"📤 Uploaded: {filename}")
                                    self._just_finished_upload = True
                                self._upload_progress_line = None
                                self._upload_progress_last_state = None
                                self.after(0, self._update_upload_progress_ui)
                                try:
                                    self.upload_progress_path.unlink()
                                except Exception:
                                    pass
                            elif state == "failed":
                                err = prog.get("error", "Unknown error")
                                self.add_activity_log(f"❌ Upload failed: {filename} — {err}")
                                self._upload_progress_line = None
                                self._upload_progress_last_state = None
                                self.after(0, self._update_upload_progress_ui)
                                try:
                    ...
                    if current_count > self.last_file_count:
                        if not self._just_finished_upload and self._upload_progress_last_state != "uploading":
